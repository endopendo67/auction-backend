...
- **Атомарные операции** — проверка и обновление в одном запросе (без транзакций!)
- **Optimistic locking** — проверка условий в update query
- **Компенсирующие операции** — автоматический откат при ошибке
- **Zero WriteConflict** — нет транзакций = нет конфликтов
...
| WriteConflict | **Невозможен** — атомарные операции |
...
> Атомарные операции без транзакций = нет WriteConflict = 100% success.
